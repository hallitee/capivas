...
class VasApiController extends MainAPIController
{
    ...
    {   
            ...
            if($text==""){
                $plan = SubscriptionPlan::where('status', 1)->where('plan_vas', 'YES')->get();
                $res = "CON Capivas Streaming Plans \n";
                foreach($plan as $k=>$p){
                    $k+=1;
                    $res.="$k. $p->plan_name for $cur".$p->plan_price."\n";
                }
            }
    ...
    { 
        ...
        if($user_info)
        {
            $response[] = array('msg' => "Adresse e-Mail déjà utilisée!",'success'=>'0'); 
    ...
    public function show_details()
    {
        ...
        $response=array("show_id"=>$series_info->id,"show_name"=>stripslashes($series_info->series_name),"show_info"=>stripslashes($series_info->series_info),"imdb_rating"=>$imdb_rating,"show_poster"=>$show_poster,'show_lang'=>$show_lang,"genre_list"=>$genre_list); 
